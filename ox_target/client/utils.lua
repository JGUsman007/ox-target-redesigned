...
local colour = vector(98, 135, 236, 255)
...
function utils.drawZoneSprites(dict, texture)
    ...
    for i = 1, drawN do
        ...
        local zonecoords = vec3(zone.coords.x, zone.coords.y, zone.coords.z)
        local playerpos = GetEntityCoords(cache.ped)
        local distance = #(playerpos - zonecoords)

        if (distance < 9) then

            local spriteColour = colour
